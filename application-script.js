...
document.addEventListener("DOMContentLoaded", function () {
    ...
    form?.addEventListener("submit", async function (e) {
        ...
        try {
            ...
            const visaFeeAED = parseFloat(
                visaFeeElement.textContent.replace("AED ", "").replace(",", ""),
            );
            const processingFeeAED =
                processingFees[formData.processingTime] || 0;
            ...
            const applicationData = {
                ...
                visaType:
                    visaTypeMapping[formData.visaType] || formData.visaType,
            ...
            try {
                ...
                console.log(
                    "Application submitted successfully to backend:",
                    result,
                );
                ...
                console.warn(
                    "Backend submission failed, using localStorage fallback:",
                    backendError,
                );
                ...
                console.log(
                    "Application saved locally with reference:",
                    applicationNumber,
                );

                // Show info message about offline mode
                showNotification(
                    "Application saved locally. It will sync when connection is restored.",
                    "info",
                );
            }
    ...
    async function uploadDocuments(applicationId) {
        ...
        if (uploadedFiles.airline) {
            documentsToUpload.push({
                file: uploadedFiles.airline,
                type: "ticket",
            });
        }
        if (uploadedFiles.hotel) {
            documentsToUpload.push({
                file: uploadedFiles.hotel,
                type: "hotel",
            });
        }
        ...
        try {
            ...
            await sdk.uploadDocuments(
                applicationId,
                documentsToUpload,
                (progress) => {
                    console.log(
                        `Upload progress: ${progress.percentage}% - ${progress.filename}`,
                    );
                },
            );
            ...
            alert(
                "Application submitted but some documents failed to upload. You can upload them later.",
            );
        }
    }
    ...
    function calculateFees() {
        ...
        // Get processing fee
        const processingFee =
            processingTime && processingFees[processingTime]
                ? processingFees[processingTime]
                : 0;
    ...
    async function fetchExchangeRates() {
        try {
            ...
            if (
                ratesLastFetched &&
                now - ratesLastFetched < 24 * 60 * 60 * 1000
            ) {
                return;
            }
